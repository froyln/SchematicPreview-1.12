...
Built so far (tasks 1-2 — skeleton, configs, hotkey, config screen, 3D preview renderer +
side-panel preview). Browser/icon/replace classes below are still planned (tasks 3-5):
...
├── render/PreviewCache.java              # Path → loaded ISchematic (async) + tessellated renderers  [done]
├── materials/BlockReplacer.java          # replace block in all region containers  [task 5]
└── mixin/                                # SchematicInfoWidgetMixin [done], BaseSchematicBrowserScreenMixin,
                                          # MaterialListScreenMixin, MaterialListSchematicAccessor,
                                          # MaterialListPlacementAccessor  [tasks 3/5]
src/main/resources/
