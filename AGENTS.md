...
- **Key dependencies:**
  ...
  - `litematica-liteloader-1.12.2` **0.31.4** (`fi.dy.masa.litematica.*`). Not on any Maven —
    the `.litemod` goes in `libs/` (flatDir + `deobfCompile`), or build it from source at
    `maruohon/litematica` commit `1db931a6` and `publishToMavenLocal`. This is the post-rewrite
    Litematica (`BaseSchematicBrowserScreen`, `SchematicInfoWidget`, `MaterialListEntryWidget`),
    NOT the 2020 `GuiSchematicBrowserBase`/`WidgetSchematicBrowser` API of older builds.
...
Built so far (task 1 — skeleton, configs, hotkey, config screen). Rendering/browser/icon/replace
classes below are still planned (tasks 2-5):
...
├── gui/PreviewWidget.java        # malilib widget: FBO preview, drag-rotate, scroll-zoom, fullscreen/freecam [task 2]
├── gui/PreviewFullscreenScreen.java  [task 2]
├── gui/DirectoryIconEditScreen.java  [task 4]
├── gui/BlockSelectScreen.java    # searchable block list for the Replace feature  [task 5]
├── gui/PreviewDirectoryEntryWidget.java  # replaces malilib DirectoryEntryWidget  [task 3]
├── gui/TileEntryWidgetFactory.java       # ListEntryWidgetFactory: N columns grid layout  [task 3]
├── gui/ReplaceMaterialListEntryWidget.java  # MaterialListEntryWidget + Replace button  [task 5]
├── render/SchematicBlockAccess.java      # IBlockAccess over ISchematic regions, full brightness  [task 2]
├── render/PreviewRenderer.java           # tessellate per BlockRenderLayer → VertexBuffer, Framebuffer  [task 2]
├── render/PreviewCache.java              # Path → loaded ISchematic (async) + small widgets  [task 2]
├── materials/BlockReplacer.java          # replace block in all region containers  [task 5]
└── mixin/                                # BaseSchematicBrowserScreenMixin, SchematicInfoWidgetMixin,
                                          # MaterialListScreenMixin, MaterialListSchematicAccessor,
                                          # MaterialListPlacementAccessor  [tasks 2/3/5]
src/main/resources/
